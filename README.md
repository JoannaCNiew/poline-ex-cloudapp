...
The app allows you to fully customize the output of your export via the Settings menu.

Open the PO Line Export app.

Click the "Settings" (gear) icon.

You can configure the following:

  - File Header: Change the text that appears as the first line in your exported file.
  
  - Select Columns: Check the boxes for the PO Line data you want to export.
  
  - Rename Columns: Change the default header for any column.
...
_I don't find PO Line Export under Installed Apps._

Find the app in the Cloud App Center and activate it.

_The app doesn't show any PO Lines, but I see them on the page._

Try refreshing the Alma page and reopening the app. This app works by reading the PO Lines currently displayed on the main Alma screen. If the list is empty, the app's list will also be empty.
...
Wybierz pożądaną akcję:
  ...
  - Podgląd: Generuje szybki podgląd danych do eksportu w oknie aplikacji.
  
  - Kopiuj: Kopiuje sformatowane dane do schowka.
...
Kliknij ikonę "Ustawienia" (koło zębate).

Możesz skonfigurować następujące elementy:

  - Nagłówek pliku: Zmień tekst, który pojawi się jako pierwsza linia w eksportowanym pliku.
  
  - Wybierz kolumny: Zaznacz pola przy tych wartościach z linii zamówienia, które chcesz eksportować.
  
  - Zmień nazwy kolumn: Zmień domyślny nagłówek dowolnej kolumny.
...
W Twojej Almie aplikacje chmurowe nie zostały włączone. Skontaktuj się z administratorem Almy, aby włączyć aplikacje chmurowe.

_Nie znajduję aplikacji PO Line Export na liście "Aktywowanych aplikacji"._

Wyszukaj aplikację w Centrum Aplikacji Chmurowych, a następnie aktywuj ją.

_Aplikacja nie pokazuje żadnych linii zamówienia, chociaż widzę je na stronie._

Spróbuj odświeżyć stronę Almy i ponownie otworzyć aplikację. Aplikacja działa poprzez odczytanie linii zamówienia aktualnie wyświetlanych na głównym ekranie Almy. Jeśli lista na stronie jest pusta, lista w aplikacji również będzie pusta.
